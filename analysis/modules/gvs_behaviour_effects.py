...
def _parse_args():
    parser = argparse.ArgumentParser(description=("Extract behaviour RT by GVS/sham blocks from run_condition_inventory.csv " "and test active GVS conditions against sham."))
    parser.add_argument("--behaviour-dir", type=Path, default=DEFAULT_BEHAVIOUR_DIR)
    parser.add_argument("--inventory", type=Path, default=DEFAULT_INVENTORY)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    parser.add_argument("--behaviour-column", type=int, default=DEFAULT_BEHAVIOUR_COLUMN_ONE_BASED, help="One-based column for 2D behaviour arrays. Default 2 matches existing medication scripts.")
    parser.add_argument("--input-is-inverse-rt", action=argparse.BooleanOptionalAction, default=True, help="Treat behaviour values as inverse RT and convert to seconds with 1/value.")
...
def _load_inventory(path):
    ...
    required = {"subject", "session", "medication", "run", "condition_code", "condition_label", "trial_start", "trial_stop", "n_trials", "run_n_trials"}
...
def _build_trial_table(inventory, behaviour_dir, behaviour_column_index, input_is_inverse_rt, min_rt, max_rt):
    ...
    return pd.DataFrame(rows).sort_values(["subject", "session", "run", "task_trial_index_zero_based"]).reset_index(drop=True), missing


def _block_summary(trials):
    rows = []
    group_cols = ["subject", "session", "medication", "run", "condition_code", "condition_label", "active_gvs", "block_position", "follows_rest", "precedes_rest"]
    for key, group in trials.groupby(group_cols, sort=True):
        ...
        record.update({"n_rt": int(values.size), "mean_rt_s": float(np.mean(values)), "mean_rt_ms": float(np.mean(values) * 1000.0), "median_rt_ms": float(np.median(values) * 1000.0), "sd_rt_ms": float(np.std(values, ddof=1) * 1000.0) if values.size > 1 else float("nan"), "mean_inverse_rt": float(np.mean(1.0 / values)), "rt_variability": variability, "n_adjacent_pairs": n_pairs})
        rows.append(record)
    return pd.DataFrame(rows).sort_values(group_cols).reset_index(drop=True)


def _aggregate_condition(blocks, group_cols):
    return (blocks.groupby(group_cols, dropna=False, sort=True) .agg(n_blocks=("mean_rt_ms", "size"), n_rt=("n_rt", "sum"), mean_rt_ms=("mean_rt_ms", "mean"), median_rt_ms=("median_rt_ms", "mean"), sd_rt_ms=("mean_rt_ms", "std"), mean_inverse_rt=("mean_inverse_rt", "mean"), rt_variability=("rt_variability", "mean"), n_adjacent_pairs=("n_adjacent_pairs", "sum")) .reset_index())
...
def _paired_stats(subject_condition, session_condition, block_condition):
    ...
    for metric in metrics:
        ...
        subject_any = (subject_delta.groupby("subject", as_index=False)[f"delta_{metric}_active_minus_sham"].mean())
        rows.append(_one_sample_stats(subject_any[f"delta_{metric}_active_minus_sham"].to_numpy(dtype=np.float64), "subject_any_active_vs_sham", metric, "subject", "any_active_minus_sham"))
        ...
        session_any = (session_delta.groupby(["subject", "session", "medication"], as_index=False)[f"delta_{metric}_active_minus_sham"].mean())
        rows.append(_one_sample_stats(session_any[f"delta_{metric}_active_minus_sham"].to_numpy(dtype=np.float64), "session_any_active_vs_sham", metric, "subject_session", "any_active_minus_sham"))
        ...
        for condition, group in subject_delta.groupby("condition_label", sort=True):
            ...
            rows.append(_one_sample_stats(values, f"subject_{condition}_vs_sham", metric, "subject", f"{condition}_minus_sham"))
        ...
        for medication, group in session_delta.groupby("medication", sort=True):
            medication_any = (group.groupby(["subject", "session", "medication"], as_index=False)[f"delta_{metric}_active_minus_sham"].mean())
            rows.append(_one_sample_stats(medication_any[f"delta_{metric}_active_minus_sham"].to_numpy(dtype=np.float64), f"{medication}_session_any_active_vs_sham", metric, "subject_session", "any_active_minus_sham"))

        block_delta = _active_minus_sham(block_condition, ["subject", "session", "medication", "run"], metric)
        block_any = (block_delta.groupby(["subject", "session", "medication", "run"], as_index=False)[f"delta_{metric}_active_minus_sham"].mean())
        rows.append(_one_sample_stats(block_any[f"delta_{metric}_active_minus_sham"].to_numpy(dtype=np.float64), "run_any_active_vs_sham", metric, "subject_session_run", "any_active_minus_sham"))
...
def _add_condition_fdr(stats_df):
    ...
    for metric, index in out.loc[condition_mask].groupby("metric", sort=False).groups.items():
        idx = list(index)
        out.loc[idx, "p_ttest_fdr_bh_condition_family"] = _bh_fdr(out.loc[idx, "p_ttest_two_sided"].to_numpy(dtype=np.float64))
        out.loc[idx, "p_wilcoxon_fdr_bh_condition_family"] = _bh_fdr(out.loc[idx, "p_wilcoxon_two_sided"].to_numpy(dtype=np.float64))
    return out
...
def _regression_tables(blocks):
    ...
    model_data["condition_label"] = pd.Categorical(model_data["condition_label"], categories=["sham"] + sorted([label for label in model_data["condition_label"].unique() if label != "sham"]))
    model_data["medication"] = pd.Categorical(model_data["medication"], categories=["OFF", "ON"])
    formulas = {"mean_rt_ms_condition_fixed_effects": "mean_rt_ms ~ C(condition_label) + C(medication) + C(run)", "rt_variability_condition_fixed_effects": "rt_variability ~ C(condition_label) + C(medication) + C(run)"}
...
def _condition_rows(stats_df, metric):
    rows = stats_df.loc[stats_df["metric"].eq(metric) & stats_df["analysis"].astype(str).str.startswith("subject_GVS")].copy()
    if rows.empty:
        return rows
...
def _draw_condition_delta_panel(ax, rows, metric, panel_label=None):
    ...
    yerr = np.vstack([means - rows["ci95_low"].to_numpy(dtype=np.float64), rows["ci95_high"].to_numpy(dtype=np.float64) - means])
    ax.errorbar(x, means, yerr=yerr, fmt="o", linestyle="none", color=color, markerfacecolor=color, markeredgecolor=color, markersize=5.8, elinewidth=1.25, capsize=3.0, capthick=1.0)
    ...
    ax.text(0.98, 0.96, _min_condition_fdr_text(rows), transform=ax.transAxes, ha="right", va="top", fontsize=8.5, bbox={"facecolor": "white", "edgecolor": "none", "alpha": 0.78, "pad": 1.5})
...
def _save_combined_delta_plot(stats_df, out_dir):
    ...
    fig.text(0.5, 0.01, "Error bars show 95% CI across subjects. FDR q values are Benjamini-Hochberg corrected across the 8 waveform tests within each panel.", ha="center", va="bottom", fontsize=8.5)
...
def _write_report(out_dir, summary, stats_df):
    primary = stats_df.loc[stats_df["analysis"].eq("subject_any_active_vs_sham") & stats_df["metric"].eq("mean_rt_ms")].iloc[0]
    variability = stats_df.loc[stats_df["analysis"].eq("subject_any_active_vs_sham") & stats_df["metric"].eq("rt_variability")].iloc[0]
    report = [
        "# GVS Behaviour Effects",
        "",
        (f"Extracted {summary['n_trials']} finite behaviour trials from {summary['n_runs']} runs, " f"{summary['n_sessions']} sessions, and {summary['n_subjects']} subjects."),
        "",
        ("Block timing uses `run_condition_inventory.csv`; each GVS/sham block contributes up to 10 task trials. " "Two 20-trial rest gaps after stimulation blocks 3 and 6 are encoded in the rest-aware timing columns, " "and RT variability is computed within stimulation blocks only."),
        "",
        "## Primary Any-Active GVS Result",
        "",
        (f"Subject-level any-active GVS minus sham mean RT delta={primary['mean_delta']:.2f} ms " f"(95% CI {primary['ci95_low']:.2f}, {primary['ci95_high']:.2f}; " f"paired t p={primary['p_ttest_two_sided']:.3g}; n={int(primary['n'])})."),
        "",
        (f"Subject-level any-active GVS minus sham RT-variability delta={variability['mean_delta']:.3f} " f"(95% CI {variability['ci95_low']:.3f}, {variability['ci95_high']:.3f}; " f"paired t p={variability['p_ttest_two_sided']:.3g}; n={int(variability['n'])})."),
        "",
        "## Figure Caption",
        "",
        (
...
def main():
    ...
    trials, _missing = _build_trial_table(inventory, args.behaviour_dir, behaviour_column_index=args.behaviour_column - 1, input_is_inverse_rt=bool(args.input_is_inverse_rt), min_rt=args.min_rt, max_rt=args.max_rt)
    blocks = _block_summary(trials)
    block_condition = _aggregate_condition(blocks, ["subject", "session", "medication", "run", "condition_code", "condition_label", "active_gvs"])
    session_condition = _aggregate_condition(blocks, ["subject", "session", "medication", "condition_code", "condition_label", "active_gvs"])
    subject_condition = _aggregate_condition(blocks, ["subject", "condition_code", "condition_label", "active_gvs"])
    ...
    summary = {
        ...
        "min_rt": args.min_rt,
        "max_rt": args.max_rt,
        "rest_gaps": {"rest_after_stim_blocks": list(REST_AFTER_STIM_BLOCKS), "rest_trials_each": REST_TRIALS, "note": "Rest-aware timing columns add 20 trials after block 3 and another 20 after block 6."},
    ...
    (args.out_dir / "gvs_behaviour_summary.json").write_text(json.dumps(_json_safe(summary), indent=2) + "\n", encoding="utf-8")
