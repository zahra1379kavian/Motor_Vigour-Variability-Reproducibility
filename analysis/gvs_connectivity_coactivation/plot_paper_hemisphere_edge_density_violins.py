...
DEFAULT_OUTPUT_BASE = (ROOT / "figures" / "GVS_effects" / "main result" / "mutual_info_quantile_vigour_task_hemisphere_significant_edge_density_by_direction_violins")
...
def load_edge_table(label, path, metric, analysis_view, fdr_scope):
    ...
    matched["hemisphere_relation"] = [hemisphere_relation(roi_i, roi_j) for roi_i, roi_j in zip(matched["roi_i"], matched["roi_j"])]
    ...
    for result_index, result_label in enumerate(result_labels):
        ...
        for direction_index, (direction_key, direction_label) in enumerate(DIRECTION_ORDER):
            for relation_index, (relation_key, relation_label) in enumerate(RELATION_ORDER):
                ...
                subset = data.loc[(data["result_set"] == result_label) & (data["hemisphere_relation"] == relation_key)].copy()
                ...
                groups.append({"result_set": result_label, "direction_key": direction_key, "direction": direction_label, "hemisphere_relation": relation_key, "relation_label": relation_label, "position": position, "n_edges_tested": int(indicator.size), "n_sig_edges": int(indicator.sum()), "observed_density": float(indicator.mean())})
    return groups
...
def density_tests(groups):
    ...
    for left, right in zip(groups[0::2], groups[1::2], strict=True):
        ...
        table = [[left_success, left_total - left_success], [right_success, right_total - right_success]]
        ...
        rows.append(
            {
                ...
                "density_difference_inter_minus_intra": float(left_success / left_total - right_success / right_total),
...
def write_group_summary(groups, output_base):
    rows = []
    for group in groups:
        rows.append({"result_set": group["result_set"], "direction": group["direction"], "hemisphere_relation": group["hemisphere_relation"], "n_sig_edges": group["n_sig_edges"], "n_edges_tested": group["n_edges_tested"], "observed_density": group["observed_density"]})
...
def write_test_summary(test_rows, output_base):
    ...
    for row in test_rows:
        export_rows.append({key: value for key, value in row.items() if key not in {"left_position", "right_position", "direction_key"}})
...
def add_test_annotation(ax, row, y_limit):
    ...
    ax.text((x_left + x_right) / 2, y + bracket_height + y_limit * 0.02, label, ha="center", va="bottom", fontsize=8.6, color="#202020", linespacing=0.95)
...
def plot_density_bars(groups, test_rows, output_base):
    ...
    ax.bar(x=positions, height=observed, width=0.58, color=colors, alpha=0.86, edgecolor="#303030", linewidth=0.8, zorder=3)
    ...
    for position, density, group, color in zip(positions, observed, groups, colors, strict=True):
        ...
        ax.text(position, density + y_limit * 0.035, count_label, ha="center", va="bottom", fontsize=8.2, color="#303030")
...
def build_parser():
    ...
    parser.add_argument("--input", nargs=2, action="append", metavar=("LABEL", "CSV"), help="Result label and full edge stats CSV. Repeat twice.")
    return parser
